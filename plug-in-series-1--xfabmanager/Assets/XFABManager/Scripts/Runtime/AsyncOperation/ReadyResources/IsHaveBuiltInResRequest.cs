...
namespace XFABManager
{
    ...
    public class IsHaveBuiltInResRequest : CustomAsyncOperation
    {

        public BuildInResType buildInResType { get; private set; }
        public bool isHave { get; private set; }

        public IEnumerator IsHaveBuiltInRes(string projectName)
        {
            yield return null;
            ...
            isHaveFiles = string.IsNullOrEmpty(requestFiles.error);

#else
            bool isHaveZip = File.Exists(zipPath);
            bool isHaveFiles = File.Exists(project_build_info);
#endif

            isHave = isHaveZip || isHaveFiles;
