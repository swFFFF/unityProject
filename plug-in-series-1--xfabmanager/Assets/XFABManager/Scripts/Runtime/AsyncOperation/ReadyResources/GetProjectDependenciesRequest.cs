...
namespace XFABManager {
    public class GetProjectDependenciesRequest : CustomAsyncOperation
    {
        ...
        public IEnumerator GetProjectDependencies(string projectName ) {
            
            if (string.IsNullOrEmpty(projectName))
            {
                _error = "projectName is empty!";
                isCompleted = true;
                yield break;
            }
            ...
            if (AssetBundleManager.Profile.updateModel == UpdateMode.UPDATE)
            {
                ...
                {
                    _error = string.Format("请求{0}版本出错!:{1}", projectName, requestVersion.error);
                    isCompleted = true;
                    yield break;
                }
                // 获取依赖项目
                ...
                {
                    _error = string.Format("获取{0}依赖出错:{1} url:{2} ", projectName, requestDepend.error,requestDepend.request_url);
                    isCompleted = true;
                    yield break;
                }
            }
